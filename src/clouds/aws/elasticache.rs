...
use super::common::{form_params, list_regions};
use super::{
    as_items,
    auth::{sign, AwsCreds},
...
async fn list_in_region(
    ...
    loop {
        ...
        let clusters = as_items(&v["DescribeCacheClustersResult"]["CacheClusters"]["CacheCluster"]);

        // Enrich each cluster with CloudWatch metrics concurrently
        let enrich_tasks: Vec<_> = clusters
            .into_iter()
            .filter_map(|c| {
                ...
                let num_nodes = c["NumCacheNodes"]
                    .as_str()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(1);
                let status = c["CacheClusterStatus"]
                    .as_str()
                    .unwrap_or("unknown")
                    .to_string();
...
async fn enrich(
    ...
    let peak_connections_14d = conn_res
        .as_ref()
        .ok()
        .and_then(|v| v.iter().cloned().reduce(f64::max).map(|m| m as u64));
...
async fn elasticache_query(
    ...
    if !status.is_success() {
        return Err(anyhow!(
            "ElastiCache API error {status} in {region}: {text}"
        ));
    }
    Ok(text)
}
